<template>
    <div>
        <el-form :model="registerForm" label-width="100px" class="registerForm" :rules="rules" ref="registerForm"
            ...
            <el-form-item label="密码:" prop="password">
                <el-input :type="(eyepassword ? 'text' : 'password')" v-model="registerForm.password"
                    placeholder="请输入密码" prefix-icon="el-icon-lock">
                    <div slot="suffix" class="icon-style"
                        :style="{'background-image': 'url(' + (eyepassword? require('./../image/show.png') : require('./../image/hide.png') )+ ')','background-size':'100%'}"
                        @click="eyepassword= !eyepassword" />
                </el-input>
            </el-form-item>
            <el-form-item label="确认密码:" prop="checkpassword">
                <el-input :type="(eyecheckpassword ? 'text' : 'password')" v-model="registerForm.checkpassword"
                    placeholder="请确认密码" prefix-icon="el-icon-lock">
                    <div slot="suffix" class="icon-style"
                        :style="{'background-image': 'url(' + (eyecheckpassword? require('./../image/show.png') : require('./../image/hide.png') )+ ')','background-size':'100%'}"
        ...
        methods: {
            submitregister() {
                if (this.registerForm.username == '' || this.registerForm.password == '' || this.registerForm
                    .checkpassword == '' || this.registerForm.email == '') {
                    this.$alert('必填项不能为空');
                } else {
                    this.$alert('注册成功');
                    this.$router.push("/");
